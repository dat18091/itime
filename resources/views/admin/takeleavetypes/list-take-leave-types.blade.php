...
@section('admin_title')
<title>IZITIME - Danh sách loại nghỉ phép</title>
@stop
@section('css')
<?php
...
<div class="container-fluid">
    ...
    <div class="row pt-2 pb-2">
        <div class="col-sm-9">
            <h4 class="page-title">DANH SÁCH LOẠI NGHỈ PHÉP</h4>
            <ol class="breadcrumb">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="action-button" style="display:flex;">
                        <div><a href="" data-toggle="modal" data-target="#addTakeLeaveType" data-whatever="@mdo" class="btn btn-success space">Tạo mới</a></div>
                        <div><a href="{{URL::to('/admin/list-take-leave-types-trash')}}" class="btn btn-primary space">Thùng rác</a></div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="example" class="table table-bordered">
                            ...
                            <tbody>
                                @foreach($list_take_leave_types as $key => $takeleavetypes)
                                <tr>
                                    <td>{{ $takeleavetypes->name }}</td>
                                    <td>{{ $takeleavetypes->note }}</td>
                                    <td>
                                        <?php
                                        if ($takeleavetypes->status == 0) {
                                        ?>
                                            <a href="{{URL::to('/admin/hide-take-leave-types/'.$takeleavetypes->id)}}"><span class="fa-styling fa fa-thumbs-up"></span></a>
                                        <?php
                                        } else {
                                        ?>
                                            <a href="{{URL::to('/admin/show-take-leave-types/'.$takeleavetypes->id)}}"><span class="fa-styling fa fa-thumbs-down"></span></a>
                                        <?php
                                        }
                                        ?>
                                    </td>
                                    <td>
                                        <div class="btn-group group-round m-1">
                                            <a type="button" href="{{URL::to('/admin/edit-take-leave-types/'.$takeleavetypes->id)}}" class="btn btn-success waves-effect waves-light">Sửa</a>
                                            <a type="button" href="{{URL::to('/admin/trash-take-leave-types/'.$takeleavetypes->id)}}" onclick="return confirm('Bạn có chắc chắn muốn xóa lý do này?')" class="btn btn-danger waves-effect waves-light">Xóa</a>
                                        </div>
                                    </td>
    ...
    <div class="modal fade bd-example-modal-lg" id="addTakeLeaveType" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Thêm Loại Nghỉ</h5>
                    ...
                    <div class="modal-body">
                        <form method="post" action="{{URL::to('/admin/save-take-leave-types')}}" id="editForm">
                            {{csrf_field()}}
                            <div class="form-group row">
                                <label for="input-14" class="col-sm-2 col-form-label">Loại nghỉ <span class="focus">*</span></label>
                                <div class="col-sm-10">
                                    <input type="text" value="" class="form-control" id="name" name="name" onkeyup="changeToKeyword();">
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <textarea class="form-control" style="resize:none;" rows="4" id="note" name="note"></textarea>
                                </div>
                            </div>
