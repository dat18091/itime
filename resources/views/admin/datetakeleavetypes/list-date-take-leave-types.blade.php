...
@section('admin_title')
<title>IZITIME - Danh sách loại ngày nghỉ phép</title>
@stop
@section('css')
<?php
            ...
            <div class="card">
                <div class="card-header">
                    <div class="action-button" style="display:flex;">
                        <div><a href="" data-toggle="modal" data-target="#addDateTakeLeaveType" data-whatever="@mdo" class="btn btn-success space">Tạo mới</a></div>
                        <div><a href="{{URL::to('/admin/list-date-take-leave-types-trash')}}" class="btn btn-primary space">Thùng rác</a></div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="example" class="table table-bordered">
                            ...
                            <tbody>
                                @foreach($list_date_take_leave_types as $key => $datetakeleave)
                                <tr>
                                    <td>{{ $datetakeleave->name }}</td>
                                    <td>{{ $datetakeleave->note }}</td>
                                    <td>
                                        <?php
                                        if ($datetakeleave->status == 0) {
                                        ?>
                                            <a href="{{URL::to('/admin/hide-date-take-leave-types/'.$datetakeleave->id)}}"><span class="fa-styling fa fa-thumbs-up"></span></a>
                                        <?php
                                        } else {
                                        ?>
                                            <a href="{{URL::to('/admin/show-date-take-leave-types/'.$datetakeleave->id)}}"><span class="fa-styling fa fa-thumbs-down"></span></a>
                                        <?php
                                        }
                                        ?>
                                    </td>
                                    <td>
                                        <div class="btn-group group-round m-1">
                                            <a type="button" href="{{URL::to('/admin/edit-date-take-leave-types/'.$datetakeleave->id)}}" class="btn btn-success waves-effect waves-light">Sửa</a>
                                            <a type="button" href="{{URL::to('/admin/trash-date-take-leave-types/'.$datetakeleave->id)}}" onclick="return confirm('Bạn có chắc chắn muốn xóa vùng này?')" class="btn btn-danger waves-effect waves-light">Xóa</a>
                                        </div>
                                    </td>
